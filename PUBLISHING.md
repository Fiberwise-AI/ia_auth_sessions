# Publishing ia-auth-sessions to PyPI

This document describes how to publish `ia-auth-sessions` to PyPI using GitHub Actions.
...
2. Push a tag starting with `v` (e.g., `v0.1.1`)

### Prerequisites

1. **Create a PyPI API token:**
   - Go to https://pypi.org/manage/account/token/
   - Create a token scoped to the `ia-auth-sessions` project
...
export TEST_PYPI_TOKEN="your-testpypi-token"
...
2. Push a test tag: `git tag test-v0.1.0 && git push origin test-v0.1.0`
3. Check the Actions tab to see if build succeeds
4. Delete test tag: `git push --delete origin test-v0.1.0 && git tag -d test-v0.1.0`
...
✅ **Fixed** - Updated `pyproject.toml` to use SPDX license expression (`license = "MIT"`).
...
- Check that the token has permissions for the `ia-auth-sessions` project
...
# Verify installation
python -c "import ia_auth_sessions; print('Successfully imported ia_auth_sessions')"

# Check on PyPI
# Visit: https://pypi.org/project/ia-auth-sessions/
```

## Rollback

PyPI doesn't allow deleting versions. To fix a bad release:
1. Increment version (e.g., 0.1.0 → 0.1.1)
